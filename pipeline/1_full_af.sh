...
# check len x seqs for size limit
len=$(awk '/^>/ {if (seen) exit; seen=1; next} {if (seen) len += length} END {print int(len)}' $seq_dir'query.fasta')
limit=$(( 2000000 / $len ))

awk -v lim=$limit 'BEGIN{FS=""}{if(NR<=2*lim){if($1==">"){if(NR==1)print $0; else {printf "\n";print $0;}}else printf toupper($0)}}' $out_dir$name".fasta" | awk 'BEGIN{FS=""}{if($1==">")name[NR]=$0; else{count=0;for(i=1;i<=NF;i++)if($i=="-")count+=1; if(count/NF<0.2){print name[NR-1]; print $0}}}' > $seq_dir'full_MSA.fasta'
