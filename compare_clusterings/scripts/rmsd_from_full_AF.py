...
def main(name, method):
    ...
    path_clusters =  base_path + name + '/AF_'+method+'/'
    output =  '../'+base_path + name + '/results_'+method+'/'
    ...
    for clu_id in sorted_clu_ids:
        ...
        path_msa = base_path + name + '/'+ method +'/' + clu_id + '.fasta'
...
def max_size_alternative_cluster(name, ids, rmsd, plddt, size, method):
    maxx = 0
    ...
    out_file_name = '../output_files/' + name + '/results_'+method+'/max_size_alternative_cluster.csv'
...
ids, rmsd, plddt, size = main(name, method)
max_size_alternative_cluster(name, ids, rmsd, plddt, size, method)
